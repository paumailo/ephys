...
case 'ExportClustersMerge'
	...
	exportType = exportType{get(exportTypeHandle, 'Value')};
	for iC = clusterStart:length(MClust_Clusters)
		if ~isa(MClust_Clusters{iC}, exportType(2:end))
            MClust_Clusters{iC} = feval(exportType(2:end), GetName(MClust_Clusters{iC}), MClust_Clusters{iC});
		end
	end
...
xlabel(MClust_FeatureNames{xdim},'interpreter','none');
ylabel(MClust_FeatureNames{ydim},'interpreter','none');
zlabel(MClust_FeatureNames{zdim},'interpreter','none');
rotate3d on
box on
% view([30 10])
