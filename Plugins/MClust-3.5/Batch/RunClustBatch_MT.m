function RunClustBatch_MT(batchfile,varargin)
%
% RunClustBatch_MT(batchfile)
%          or
...
if StartAtStep <= 1
    ...
    for iTTfn = 1:nFiles
        ...
        TotalFeatures(cell2mat(cellfun(@isempty,TotalFeatures,'UniformOutput',false))) = []; % DJS 5/2013
...
if StartAtStep <= 3 & strcmp(Do_AutoClust,'yes')
    ...
    for iCrFDs = 1:nFiles
        ...
        if CalcFD
            ...
            UseFD_index{iCrFDs} = ones(1,size(FeatureData,2));
            disp([' Wrote ' fnameout ' as a .mat formatted file']);
        else
            gPar.SubsampledFileNames{iCrFDs} = FDTextFname;
            UseFD_index{iCrFDs} = ones(1,size(temp.FeatureData,2));
...
if strcmp(Do_AutoClust,'yes')
    ...
    FinishedFiles = zeros(nFiles,1);
    IN = gPar.SubsampledFileNames{1};
    [INpath INname INext] = fileparts(IN);
    pushdir(INpath);
    if exist('FinishedFiles.mat','file'), load('FinishedFiles.mat');    end
    popdir;
    ...
    for i = 1:nFiles
        ...
        CurrHour = str2num(CurrTime(end-7:end-6)); %#ok<ST2NM>
...
%         end
        ...
        elseif(strcmpi(gPar.ClusterAlgorithm,'KlustaKwik'))
            ...
            parameter_string = ['-MinClusters ' num2str(fPar{jF}.KKwikMinClusters) ...
                ...
                ' -MaxPossibleClusters ' num2str(fPar{jF}.KKwikMaxClusters) ...
                ' -FullStepEvery 10' ...
                ' -UseFeatures ' num2str(UseFD_index{i}')'];
            ...
            [S,R] = system([COMMAND ' &']); %#ok<NASGU,ASGLU>
        end
