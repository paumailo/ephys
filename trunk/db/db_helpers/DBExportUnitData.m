...
[info,data,groups,params] = GetDBdata(blockid);
...
for u = units
    ...
    for i = 1:n
        fprintf(fid,'\t%d\t"%s"\t%d\t', ...
            info.(u).id,char(info.(u).pool),info.(u).count);
        for p = params
            p = char(p); %#ok<FXSET>
...
function finfo = FormatDatasetInfo(info)
finfo = sprintf('Experiment:\t%s\n',info.expt_name);
...
function [info,data,groups,params] = GetDBdata(blockid)
...
for fn = fieldnames(info)'
    fn = char(fn); %#ok<FXSET>
    if iscellstr(info.(fn))
        info.(fn) = char(info.(fn));
    end
end
       
chandata = mym(['SELECT v.channel AS channel_id, v.unit as unit_id, ', ...
           'c.channel, u.pool, u.unit_count, u.note, u.isbad, u.in_use ', ...
           'FROM v_ids v JOIN channels c ON c.id = v.channel ', ...
           'LEFT OUTER JOIN units u ON u.id = v.unit ', ...
           'WHERE v.block = {Si}'],blockid);
