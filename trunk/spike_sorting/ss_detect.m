...
if ~isfield(spikes.params,'rejectthresh'), spikes.params.rejectthresh = false; end % DJS 11/2010
...
%         progress_bar(j/num_trials); % BA
    ...
    if isnumeric(spikes.params.rejectthresh) % DJS 11/2010
        rejind = false(size(w,1),size(w,3));
        for k = 1:size(w,3)
            rejind(:,k) = any(w(:,:,k) < spikes.params.rejectthresh(k),2);
        end
        w(any(rejind,2),:,:) = [];
        spikes.spiketimes(any(rejind,2)) = [];
        spikes.trials(any(rejind,2))     = [];
